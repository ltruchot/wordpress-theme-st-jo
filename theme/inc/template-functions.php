...
/**
 * Returns the main navigation items, keyed by page slug.
 *
 * The header used to repeat this list twice -- once for the desktop nav, once
 * for the mobile one -- so every change had to be made in both places, and the
 * current page was flagged with a class but never announced to assistive tech.
 */
function st_jo_nav_items() {
	return array(
		'ecole'                  => "L'école",
		'services-periscolaires' => 'Services périscolaires',
...
/**
 * Prints the main navigation list.
 *
 * Links carry the trailing slash WordPress canonicalises to: without it every
 * internal click and every crawl pays a 301.
 */
function st_jo_nav_list() {
	echo '<ul>';

	foreach ( st_jo_nav_items() as $slug => $label ) {
		$is_current = is_page( $slug );

		printf(
			'<li><a href="%1$s"%2$s%3$s>%4$s</a></li>',
			esc_url( home_url( '/' . $slug . '/' ) ),
			$is_current ? ' class="current-menu-item"' : '',
...
function st_jo_get_the_archive_title() {
	if ( is_category() ) {
		$title = __( 'Archives de la catégorie : ', 'st-jo' ) . '<span>' . single_term_title( '', false ) . '</span>';
	} elseif ( is_tag() ) {
		$title = __( 'Archives de l’étiquette : ', 'st-jo' ) . '<span>' . single_term_title( '', false ) . '</span>';
	} elseif ( is_author() ) {
		$title = __( 'Archives de l’auteur : ', 'st-jo' ) . '<span>' . get_the_author_meta( 'display_name' ) . '</span>';
	} elseif ( is_year() ) {
		$title = __( 'Archives de l’année : ', 'st-jo' ) . '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'st-jo' ) ) . '</span>';
	} elseif ( is_month() ) {
		$title = __( 'Archives du mois : ', 'st-jo' ) . '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'st-jo' ) ) . '</span>';
	} elseif ( is_day() ) {
		$title = __( 'Archives du jour : ', 'st-jo' ) . '<span>' . get_the_date() . '</span>';
	} elseif ( is_post_type_archive() ) {
		$cpt   = get_post_type_object( get_queried_object()->name );
		$title = sprintf(
			/* translators: %s: Post type singular name */
			esc_html__( 'Archives : %s', 'st-jo' ),
